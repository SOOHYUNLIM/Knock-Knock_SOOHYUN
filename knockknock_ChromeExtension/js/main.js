...
$(document).ready(function () {
        ...
        getCurrentTabUrl(url => new AjaxBuilder().url("check").method(Method.GET).data({url: encodeURIComponent(url)})
            .success((data, status, xhr) => {
                if(xhr.status === HttpStatus.OK){
                    ...
                    $centerContent.html('<input type="text" class="wantedTitle" placeholder="1~20자 사이로 입력해주세요.."><span class="naverBtn">찾기</span>')
                }
            })
            .build())
    })
    ...
    $(".content").on('click', '.pickBtn', function (event) {
        ...
        new AjaxBuilder().url("getInterestList").method(Method.GET)
            .success(data=>{
                ...
                $(".centerInterest").html('<p id="saveInterestBtn">저장</p>')
        ...
        new AjaxBuilder().url("pickList").method(Method.GET).success(list =>
            Array.from(list).forEach(item => item.pno % 2 == 0 ? appendLeftContent("#pickListTemplate", item) : appendRightContent("#pickListTemplate", item))
        ).build()
    ...
    }).on('click', '#saveInterestBtn', event => {
        let interests = new Array();
        $("input:checkbox[name=category]:checked").each((index, item) => interests.push(item.value))
        interests.length > 0 ? new AjaxBuilder().method(Method.PUT).url("updateInterest").data(JSON.stringify(interests)).success(status => {alert('설정 완료!'); $interest.css('display', 'none') }).build() : alert('하나 이상 선택해주세요!')
    })
